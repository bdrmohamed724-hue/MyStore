    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-md border-b border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          {/* Logo */}
          <Link href="/" className="text-lg sm:text-xl font-bold tracking-[0.3em] uppercase text-white hover:text-white/80 transition-colors">
            RYVEN DEPT.
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block relative group">
              <button className="text-xs tracking-wider uppercase text-white/70 hover:text-white transition-colors">
                {locale.toUpperCase()}
              </button>
        ...
        <div className="md:hidden animate-slideDown bg-black/95 backdrop-blur-lg border-t border-white/5">
          <nav className="px-6 py-6 space-y-4">
            ...
            <div className="pt-4 border-t border-white/10">
              <p className="text-xs text-white/40 uppercase tracking-wider mb-3">{t("language")}</p>
                ...
                {locales.map((l) => (
                  <button
                    ...
                    className={`text-sm px-3 py-1 rounded transition-colors ${
                      locale === l ? "bg-white text-black" : "text-white/60 hover:text-white border border-white/20"
                    }`}
                  >
                    {l.toUpperCase()}
                  </button>
