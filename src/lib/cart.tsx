...
function cartReducer(state: CartState, action: CartAction): CartState {
  switch (action.type) {
    case "ADD": {
      ...
      return { items: [...state.items, { ...action.item, quantity: Math.min(action.item.quantity, action.item.stock) }] };
    }
    case "REMOVE":
    ...
    case "SET_QTY":
      return {
        items: state.items.map((i) =>
          i.productId === action.productId ? { ...i, quantity: Math.max(1, Math.min(action.quantity, i.stock)) } : i
        ),
      };
    case "CLEAR":
      return { items: [] };
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <CartContext.Provider value={{ ...state, addItem, removeItem, setQuantity, clearCart, subtotal, itemCount }}>
      {children}
